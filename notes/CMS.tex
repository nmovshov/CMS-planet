...
\end{equation}
\red{Assuming for now that the infinite series on $k$ converges} we can change the
...
integral will have a term that is simply a constant times $\ptk(\mu')$. This term
is identically zero except when $k=0$, so we will also need to do the $k=0$ term
separately. Third, we will lump an $r_j^2$ term together with the contribution
from $V_{i<j,\mathrm{ext}}$. Also we will use the oblate symmetry to take the
integrals over $\mu$ from $0$ to $1$. Here we go.
...
\begin{subequations}\label{eq:viltj}
\begin{equation}
...
%potential at point $B$ on the surface of spheroid $j$:
%\begin{equation}
%V_B(\xi_j,\mu) = -\frac{GM}{a_0}\frac{1}{\xi_j(\mu)} \left[
